...
async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    // const db = client.db("timeline");
    const showV = Object.keys(showId)[0];
    const showValue = showId[showV];
    ...
    try {
      console.log("getTRY");
      let document = await getDocument(client, "shows", { showId: JSON.parse(showValue) }); // maybe add this to filter at the end: , { showId: showId }
