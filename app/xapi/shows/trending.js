...
async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    let shows
    /*  try {
      let documents = await getSelectedDocuments(client, "shows", { playCount: -1 }); // maybe add this to filter at the end: , { showId: showId }
      ...
      // console.log("api-documents", docIds);
    } catch (error) {
      res.status(500).json({ message: "Fetching shows failed" });
    } */
    // Below is the remodelled (by ChatGPT) try block - above is the original - any probs, switch back!
    try {
      let documents = await getSelectedDocuments(
        client,
        'shows',
        { playCount: -1, _id: -1 },
        { playCount: { $exists: true } }
      )
      res.status(200).json({ shows: documents })
    } catch (error) {
      res.status(500).json({ message: 'Fetching shows failed' })
